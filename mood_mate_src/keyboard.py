...
BUTTONS_TEXT_LANG = {
    Language.RU.value: {
        ...
        "get_plot": "Хочу весь график! 📈",
        "get_plot_7_days": "График за последние 7 дней 📈",
        "get_plot_30_days": "График за последние 30 дней 📈",
        "get_plot_60_days": "График за последние 60 дней 📈",
    ...
    Language.ENG.value: {
        ...
        "get_plot": "Get the whole plot! 📈",
        "get_plot_7_days": "Plot for the last 7 days 📈",
        "get_plot_30_days": "Plot for the last 30 days 📈",
        "get_plot_60_days": "Plot for the last 60 days 📈",
...
def get_inline_keyboard_buttons_from_list(
    options: list[str], callback_group: str, picked_options: list[str] = list()
    ...
    for option in options:
        if option in picked_options:
            keyboard_buttons.append(
                [
                    InlineKeyboardButton(
                        text=f"{option} ✅", callback_data=f"{callback_group}_{option}"
                    )
                ]
            )
        else:
            keyboard_buttons.append(
                [
                    InlineKeyboardButton(
                        text=option, callback_data=f"{callback_group}_{option}"
                    )
                ]
            )
    ...
    accept_button = InlineKeyboardButton(
        text=BUTTONS_TEXT_LANG[Language.ENG.value]["accept"],
...
def get_start_keyboard(user: User | None = None):
    ...
    keyboard = types.ReplyKeyboardMarkup(
        keyboard=keyboard_buttons, resize_keyboard=True, input_field_placeholder="ТЫК"
    )
    return keyboard
...
def get_inline_settings_keyboard(user: User | None = None) -> InlineKeyboardBuilder:
    ...
    inline_keyboard = [
        [
            InlineKeyboardButton(
                text=BUTTONS_TEXT_LANG[language]["change_language"],
                callback_data="change_language",
            )
        ],
        [
            InlineKeyboardButton(
                text=BUTTONS_TEXT_LANG[language]["set_recommended_sleep"],
                callback_data="set_recommended_sleep",
            ),
        ],
        [
            InlineKeyboardButton(
                text=BUTTONS_TEXT_LANG[language]["toggle_reminder"],
                callback_data="toggle_reminder",
            ),
        ],
        [
            InlineKeyboardButton(
                text=BUTTONS_TEXT_LANG[language]["set_assistant_role"],
                callback_data="set_assistant_role",
            ),
        ],
        [
            InlineKeyboardButton(
                text=BUTTONS_TEXT_LANG[language]["set_ai_model"],
    ...
    if user is not None:
        if user.settings.weekly_report_enabled:
            inline_keyboard.append(
                [
                    InlineKeyboardButton(
                        text=BUTTONS_TEXT_LANG[language]["toggle_weekly_report_off"],
            ...
            inline_keyboard.append(
                [
                    InlineKeyboardButton(
                        text=BUTTONS_TEXT_LANG[language]["toggle_weekly_report_on"],
...
class EmojiSet:
    ...
    ) -> None:  # Dict with language keys and comments as values
    ...
    def get_inline_keyboard_buttons(
        self, callback_group: str | None = None
    ) -> list[InlineKeyboardButton]:
    ...
    def get_keyboard_buttons(self):
        ...
        keyboard_buttons = [[KeyboardButton(text=emoji) for emoji in self.emoji_set]]
        return keyboard_buttons
...
emotional_emoji_sets = {
    # For mood
    "mood": EmojiSet(
        emoji_set=["😭", "😢", "😟", "😐", "🙂", "😃", "😄"], data_type_names=["mood"]
    ),
    # For horny level:
    "horny": EmojiSet(
        emoji_set=["😐", "🙂", "😏", "😍", "😈", "🔥"],
