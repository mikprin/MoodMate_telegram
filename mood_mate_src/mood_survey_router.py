...
from mood_mate_src.ai_agent.suggestions import get_ai_reaction_to_records_async
from mood_mate_src.database_tools.mood_data import (
    MoodData, MoodRecord, add_mood_record_to_db,
...
async def no_note_callback_handler(query: types.CallbackQuery, state: FSMContext):
    ...
    await query.answer()
    last_week_records = get_user_records_for_past_time(user.user_id, 7*60*60*24)

    ai_response = await get_ai_reaction_to_records_async(user, last_week_records)
